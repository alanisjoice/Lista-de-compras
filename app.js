...
async function carregarLista() {
  const { data, error } = await supabase.from('lista-de-compras-araujo').select('*')
  if (error) {
...
window.adicionarItem = async function () {
  ...
  const { error } = await supabase.from('lista-de-compras-araujo').insert({
    item: input.value,
...
window.removerItem = async function (id) {
  const { error } = await supabase.from('lista-de-compras-araujo').delete().eq('id', id)
